...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    selected_paragrapgs = [p for p in paragraphs if select(p)] 
    return selected_paragrapgs[k] if len(selected_paragrapgs) > k else ''
    # END PROBLEM 1
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    return lambda p: any([remove_punctuation(lower(p)) for p in split(p) if remove_punctuation(lower(p)) in topic])
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    if not typed and reference:
        return 0.0
    return sum([typed_words[i] == reference_words[i] for i in range(reference_len) if i < reference_len and i < typed_len]) / typed_len * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    diff_words = [[word, diff_function(word, typed_word, limit)] for word in word_list if diff_function(word, typed_word, limit) <= limit]
    if not diff_words:
        return typed_word
    else:
        return min(diff_words, key=lambda w: w[1])[0]
    # END PROBLEM 5
...
def minimum_mewtations(start, goal, limit):
    ...
    if limit < 0:  # Fill in the condition
        ...
        return 0
    elif (not start_len and goal_len) or (start_len and not goal_len):
        return abs(start_len - goal_len)
    elif start[0] == goal[0]:
        return minimum_mewtations(start[1:start_len], goal[1:goal_len], limit)
    else:
        add = 1 + minimum_mewtations(goal[0] + start, goal, limit - 1)
        remove = 1 + minimum_mewtations(start[1:start_len], goal, limit - 1)
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    times = [[time[i] - time[i - 1] for i in range(1, len(time))] for time in times_per_player]
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    # look at each word -> [(0, 5, 0), (1, 4, 0)] 
    # (player_index, player_time, word_index) 0, 1, 2
    fastest_records = [[] for _ in player_indices]
    for w_index in word_indices:
        fastest = min([(p_index, match['times'][p_index][w_index], w_index) for p_index in player_indices], key=lambda t: t[1])
        player_index = fastest[0]
        word_index = fastest[2]
        fastest_records[player_index].append(word_at(match, word_index))
