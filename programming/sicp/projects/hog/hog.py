...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        ...
        if score == 1:
            sow_sad = True
        sum_score += score
    return SOW_SAD_SCORE if sow_sad else sum_score
    # END PROBLEM 1
...
def hefty_hogs(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    if opponent_score == 0:
        return SOW_SAD_SCORE
    updated_score = player_score
    while opponent_score > 0:
        digit = opponent_score % 10
        updated_score = digit_fn(digit)(updated_score)
        opponent_score = opponent_score // 10
    return updated_score % 30
    # END PROBLEM 2
