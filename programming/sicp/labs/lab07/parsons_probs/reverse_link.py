def reverse_link(lnk):
    ...
    lnk_copy = lnk
    reversed_link = None
    while not lnk_copy.rest is Link.empty:
        if not reversed_link:
            reversed_link = Link(lnk_copy.first)
        else:
            reversed_link = Link(lnk_copy.first, reversed_link)
        lnk_copy = lnk_copy.rest
    
    return Link(lnk_copy.first, reversed_link)
