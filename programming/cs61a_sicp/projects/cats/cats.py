...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    if len(paragraphs) > k:
        selected_paragraph = paragraphs[k]
        if select(selected_paragraph):
            return selected_paragraph
        else:
            paragraphs.pop(k)
            return pick(paragraphs, select, k)
    else:
        return ''
    # END PROBLEM 1
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    return lambda p: any([remove_punctuation(lower(word)) in subject for word in split(p)])
    # END PROBLEM 2
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        word_diff = [diff_function(typed_word, w, limit) for w in word_list if diff_function(typed_word, w, limit) <= limit]
        if not word_diff:
            return typed_word
        min_diff = min(word_diff, key=abs)
        corrected_words = [w for w in word_list if diff_function(typed_word, w, limit) == min_diff] 
        return  corrected_words[0] if corrected_words else typed_word
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    if not typed and not source:
        return 0
    elif (not typed and source) or (not source and typed):
        return abs(len(typed) - len(source))
    elif limit < 0:
        return 99999999
    else:
        change = 1 if typed[0] != source[0] else 0
        limit = limit if not change else limit - 1
...
def minimum_mewtations(typed, source, limit):
    ...
    if limit <= 0:
        ...
        add = 1 + minimum_mewtations(source[0]+typed, source, limit-1) if typed[0] != source[0] else 0
        remove = 1 + minimum_mewtations(typed[1:], source, limit-1) if typed[0] != source[0] else 0
        substitute = 1 + minimum_mewtations(source[0]+typed[1:], source, limit-1) if typed[0] != source[0] else 0
        if source[0] == typed[0]:
...
def time_per_word(words, timestamps_per_player):
    ...
    for time in timestamps_per_player:
        time = [time[i+1] - time[i] for i in range(len(time)) if i < len(time)-1]
        times.append(time)
...
def fastest_words(match):
    ...
    for w_index in word_indices:
        players_word_times = [{'player': p_num, 'time': time(match, p_num, w_index)} for p_num in player_indices]
        word_times = [pt['time'] for pt in players_word_times]
        if all([t == word_times[0] for t in word_times]):
            words[0].append(get_word(match, w_index))
        else:
