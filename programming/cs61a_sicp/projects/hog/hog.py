...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while i <= num_rolls:
        ...
        outcomes += curr
        i += 1
    return outcomes if not sow_sad else 1
    # END PROBLEM 1
...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    p_rmd = player_score % 10
    opp_td = opponent_score // 10 % 10 if opponent_score // 10 > 10 else opponent_score // 10
    return 3 * abs(opp_td - p_rmd) or 1 
    # END PROBLEM 2
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    avg_dice = make_averaged(roll_dice, samples_count)
    num_roll, highest_avg = 1, avg_dice(1, dice)
    for i in range(2, 11):
        curr_avg = avg_dice(i, dice)
        if curr_avg > highest_avg:
            num_roll, highest_avg = i, curr_avg
    return num_roll
    # END PROBLEM 9
