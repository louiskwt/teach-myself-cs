def filter(condition, lst):
    ...
    [2, 0]
    """
    filtered_lst = [item for item in lst if not condition(item)]
    for item in filtered_lst:
        lst_index = lst.index(item)
        lst.remove(lst[lst_index])
...
def max_path_sum(t):
    ...
    17
    """
    sum = [max_path_sum(b) for b in branches(t)] if not is_leaf(t) else [0]
    return label(t) + max(sum)
